...
if (edtHost) edtHost->hide();
if (edtPort) edtPort->hide();
if (btnConnect) btnConnect->setText(QStringLiteral("Thu ket noi lai"));
...
void AuthWindow::setStatus(const QString& text) {
    if (lblStatus) lblStatus->setText(text);
}
...
void AuthWindow::updateUiState() {
    ...
    if (btnConnect) btnConnect->setEnabled(!m_busy && !m_connected);
    if (edtHost) edtHost->setEnabled(!m_busy && !m_connected);
    if (edtPort) edtPort->setEnabled(!m_busy && !m_connected);

    const bool canAuth = m_connected && !m_busy;
    if (btnLogin) btnLogin->setEnabled(canAuth);
    if (btnRegister) btnRegister->setEnabled(canAuth);

    if (edtLoginUser) edtLoginUser->setEnabled(!m_busy);
    if (edtLoginPass) edtLoginPass->setEnabled(!m_busy);

    if (edtRegUser) edtRegUser->setEnabled(!m_busy);
    if (edtRegPass) edtRegPass->setEnabled(!m_busy);
    if (edtRegPass2) edtRegPass2->setEnabled(!m_busy);
}

void AuthWindow::onConnectClicked() {
    if (!m_client || m_busy) return;
...
void AuthWindow::onLineReceived(const QString& line) {
    ...
    if (tok[0] == "ERR") {
        ...
        if (m_pending == PendingAction::Login) title = QStringLiteral("Đăng nhập thất bại");
        else if (m_pending == PendingAction::Register) title = QStringLiteral("Đăng ký thất bại");
    ...
    if (tok[0] == "OK") {
        ...
        if (!this->isVisible() || m_pending == PendingAction::None) return;
        ...
        if (tok.size() >= 3 && tok[1] == "REGISTERED") {
            ...
            if (edtRegPass) edtRegPass->clear();
            if (edtRegPass2) edtRegPass2->clear();
