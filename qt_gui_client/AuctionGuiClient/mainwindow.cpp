...
static QString joinTokens(const QStringList& tok, int start) {
    QStringList out;
    for (int i = start; i < tok.size(); ++i) out << tok[i];
    return out.join(' ');
}

static QString translateServerErr(const QStringList& tok, const QString& raw) {
    // Expected: ERR <CODE> <message...>
    if (tok.size() < 2) return QStringLiteral("Có lỗi xảy ra (phản hồi không hợp lệ từ máy chủ).");
    ...
    if (map.contains(code)) return map.value(code);
    ...
    if (code == "BID_TOO_LOW") {
        ...
        if (pos >= 0) {
            ...
            if (!minv.isEmpty()) return QStringLiteral("Giá đặt quá thấp. Tối thiểu: %1").arg(minv);
        }
    ...
    if (code == "INVALID" || code == "INVALID_PRICE" || code == "INVALID_ITEM_FORMAT") {
        ...
        if (d.contains("duration") && d.contains(">= 10")) {
            return QStringLiteral("Thời lượng phải tối thiểu 10 giây.");
        }
        if (d.contains("buy_now") && d.contains(">= start")) {
...
static quint16 defaultPort() {
    ...
    if (ok && p > 0 && p <= 65535) return (quint16)p;
    return 8080;
}
...
static QString statusToVi(const QString& code) {
    ...
    if (c == "PENDING" || c == "ROOM_PENDING") return QStringLiteral("Chờ");
    if (c == "STARTED" || c == "ROOM_STARTED") return QStringLiteral("Đang đấu giá");
    if (c == "ENDED"   || c == "ROOM_ENDED")   return QStringLiteral("Đã kết thúc");
    if (c == "NONE") return QStringLiteral("-");
...
static QString comboStatusWantedCode(int idx) {
    ...
    switch (idx) {
    case 1: return QStringLiteral("PENDING");
    case 2: return QStringLiteral("STARTED");
    case 3: return QStringLiteral("ENDED");
...
if (ui->edtHost) ui->edtHost->setText(defaultHost());
if (ui->spinPort) ui->spinPort->setValue((int)defaultPort());

// Ẩn host/port nếu bạn muốn UI gọn hơn
if (ui->edtHost) ui->edtHost->hide();
if (ui->spinPort) ui->spinPort->hide();
if (ui->lblHost) ui->lblHost->hide();
if (ui->lblPort) ui->lblPort->hide();
if (ui->btnConnect) ui->btnConnect->setText("Thu ket noi lai");
    ...
    if (ui->lblChatPeer) ui->lblChatPeer->setText("Đang chat với: -");
    if (ui->btnChatSend) ui->btnChatSend->setEnabled(false);
...
QTimer::singleShot(0, this, [this]() {
    ...
    // Chat
    if (ui->btnRefreshChatUsers) connect(ui->btnRefreshChatUsers, &QPushButton::clicked, this, &MainWindow::onRefreshChatUsers);
    if (ui->btnChatUsers) connect(ui->btnChatUsers, &QPushButton::clicked, this, &MainWindow::onRefreshChatUsers);
    if (ui->btnChatSend) connect(ui->btnChatSend, &QPushButton::clicked, this, &MainWindow::onChatSend);
    if (ui->edtChatMsg) connect(ui->edtChatMsg, &QLineEdit::returnPressed, this, &MainWindow::onChatSend);
...
bool MainWindow::suppressDuplicateNotice(const QString& key, int windowMs) {
    ...
    if (m_noticeLastMs.size() > 256) {
        ...
        for (int i = 0; i < removeCount; ++i) m_noticeLastMs.remove(items[i].first);
    }
    return false;
}
...
void MainWindow::ensureRoomChatBar() {
    if (m_roomChatEdit && m_roomChatSendBtn) return;
    if (!ui->vLeft || !ui->pageRoom) return;
...
void MainWindow::setRoomChatEnabled(bool on) {
    ...
    if (m_roomChatEdit) {
        ...
        if (!on) m_roomChatEdit->clear();
    }
    if (m_roomChatSendBtn) m_roomChatSendBtn->setEnabled(on);
}
...
void MainWindow::syncCountdownFromMmss(const QString& mmss) {
    ...
    m_timeLeftSec = sec;
    if (ui->lblItemTime) ui->lblItemTime->setText(formatSecondsToMmss(m_timeLeftSec));
    if (m_countdownTimer && m_timeLeftSec > 0) m_countdownTimer->start();
    if (m_countdownTimer && m_timeLeftSec <= 0) m_countdownTimer->stop();
}
...
void MainWindow::openRoomTab() {
    if (!ui->tabHome || m_tabRoomIndex < 0) return;
...
void MainWindow::closeRoomTab(bool switchToRoomsTab) {
    ...
    if (ui->btnLeaveRoom) ui->btnLeaveRoom->setText("Rời phòng");
    // Re-enable các nút thao tác phòng (phòng thường)
    if (ui->btnStartRoom) ui->btnStartRoom->setEnabled(true);
    if (ui->btnEndRoom) ui->btnEndRoom->setEnabled(true);
    if (ui->btnViewQueued) ui->btnViewQueued->setEnabled(true);
    if (ui->btnViewEnded) ui->btnViewEnded->setEnabled(true);
    if (ui->btnAddDraft) ui->btnAddDraft->setEnabled(true);
    if (ui->btnDraftList) ui->btnDraftList->setEnabled(true);
    if (ui->btnPublishOk) ui->btnPublishOk->setEnabled(true);
    if (ui->btnBid) ui->btnBid->setEnabled(true);
    if (ui->btnBuyNow) ui->btnBuyNow->setEnabled(true);
    if (ui->edtBidAmount) ui->edtBidAmount->setEnabled(true);
    if (ui->edtDraftName) ui->edtDraftName->setEnabled(true);
    if (ui->edtDraftStart) ui->edtDraftStart->setEnabled(true);
    if (ui->edtDraftBuy) ui->edtDraftBuy->setEnabled(true);
    if (ui->edtDraftDur) ui->edtDraftDur->setEnabled(true);
    if (ui->tabHome && m_tabRoomIndex >= 0) {
        ui->tabHome->setTabEnabled(m_tabRoomIndex, false);
        if (switchToRoomsTab) ui->tabHome->setCurrentIndex(0);
    }
    ...
    // chat reset
    if (ui->lstRoomUsers) ui->lstRoomUsers->clear();
    if (ui->lstChatUsers) ui->lstChatUsers->clear();
    if (ui->txtChat) ui->txtChat->clear();
    m_chatPeer.clear();
    if (ui->lblChatPeer) ui->lblChatPeer->setText("Đang chat với: -");
    if (ui->btnChatSend) ui->btnChatSend->setEnabled(false);
...
void MainWindow::onJoinSelected() {
    ...
    if (stCode.isEmpty()) stCode = get(2).trimmed();
    ...
    if (stCode == "ENDED" && isOwner) {
        ...
        if (ui->btnLeaveRoom) ui->btnLeaveRoom->setText("Đóng");
        if (ui->btnStartRoom) ui->btnStartRoom->setEnabled(false);
        if (ui->btnEndRoom) ui->btnEndRoom->setEnabled(false);
        if (ui->btnViewQueued) ui->btnViewQueued->setEnabled(false);
        // ViewEnded vẫn bật để refresh kết quả
        if (ui->btnAddDraft) ui->btnAddDraft->setEnabled(false);
        if (ui->btnDraftList) ui->btnDraftList->setEnabled(false);
        if (ui->btnPublishOk) ui->btnPublishOk->setEnabled(false);
        if (ui->btnBid) ui->btnBid->setEnabled(false);
        if (ui->btnBuyNow) ui->btnBuyNow->setEnabled(false);
        if (ui->edtBidAmount) ui->edtBidAmount->setEnabled(false);
        if (ui->edtDraftName) ui->edtDraftName->setEnabled(false);
        if (ui->edtDraftStart) ui->edtDraftStart->setEnabled(false);
        if (ui->edtDraftBuy) ui->edtDraftBuy->setEnabled(false);
        if (ui->edtDraftDur) ui->edtDraftDur->setEnabled(false);
...
void MainWindow::onRoomsSelectionChanged() {
    ...
    if (items.isEmpty()) {
        ...
        if (ui->edtSelRoomUserSearch) ui->edtSelRoomUserSearch->clear();
    ...
    auto itSt = ui->tblRooms->item(row, 2);
    QString stCode = itSt ? itSt->data(Qt::UserRole).toString().trimmed() : QString();
    if (stCode.isEmpty()) stCode = get(2); // fallback
...
void MainWindow::onAddDraft() {
    QString name = ui->edtDraftName->text().trimmed();
    if (name.isEmpty()) {
        QMessageBox::warning(this, "Thiếu thông tin", "Cần nhập tên nháp/vật phẩm.");
        return;
    }
    name.replace(' ', '_');

    const QString start = ui->edtDraftStart->text().trimmed();
    const QString buy   = ui->edtDraftBuy->text().trimmed().isEmpty() ? "0" : ui->edtDraftBuy->text().trimmed();
    const QString dur   = ui->edtDraftDur->text().trimmed();

    if (start.isEmpty() || dur.isEmpty()) {
        QMessageBox::warning(this, "Thiếu thông tin", "Cần nhập giá khởi điểm và thời lượng.");
        return;
    }

    sock->sendLine(QString("ADD_ITEM %1|%2|%3|%4").arg(name, start, buy, dur));

    ui->edtDraftName->clear();
    ui->edtDraftStart->clear();
    ui->edtDraftBuy->clear();
    ui->edtDraftDur->clear();
}
...
void MainWindow::onLine(const QString& line) {
    ...
    if (tok[0] == "OK") handleOK(tok, line);
    else if (tok[0] == "ERR") handleERR(tok, line);
    else if (tok[0] == "EVT") handleEVT(tok, line);
}
...
void MainWindow::applyRoomFilter() {
    ...
    for (int r = 0; r < ui->tblRooms->rowCount(); ++r) {
        ...
        if (itSt) {
            ...
            if (stCode.isEmpty()) stCode = stDisp;
        }
        ...
        if (scope == 0) {
            if (stCode == "ENDED") ok = false;
            if (!userNorm.isEmpty() && ownerNorm.compare(userNorm, Qt::CaseInsensitive) == 0) ok = false;
        }

        // scope == 1: phòng của tôi -> chỉ owner = username (bao gồm ENDED)
        if (scope == 1 && !username.isEmpty()) {
            if (ownerNorm.compare(userNorm, Qt::CaseInsensitive) != 0) ok = false;
        }

        if (!statusWanted.isEmpty()) {
            if (stCode != statusWanted) ok = false;
        }
...
void MainWindow::handleOK(const QStringList& tok, const QString& raw) {
    ...
    if (tok.size() >= 2 && tok[1] == "ROOM_CREATED") {
        ...
        if (ui->edtCreateRoom) ui->edtCreateRoom->clear();
...
void MainWindow::handleEVT(const QStringList& tok, const QString& raw) {
    ...
    if (ev == "ROOMS_CHANGED") {
        if (m_hasSession) onRefreshRooms();
        return;
    }
    ...
    if (ev == "ALERT_30S") {
        ...
        if (tok.size() >= 4) {
            bool okId = false;
            const int iid = tok[2].toInt(&okId);
            if (okId && iid == currentItemId) syncCountdownFromMmss(tok[3]);
        }
        return;
    }
    ...
    if (ev == "QUEUED") {
        if (tok.size() >= 7) queueAddRow({tok[2], tok[3], tok[4], tok[5], tok[6]});
        return;
    }
    if (ev == "QUEUED_END") {
    ...
    if (ev == "DRAFT_ITEM" || ev == "DRAFT") {
        if (tok.size() >= 7) draftAddRow({tok[2], tok[3], tok[4], tok[5], tok[6]});
        return;
    }
    if (ev == "PUBLISHED") {
    ...
    if (ev == "ENDED") {
        if (tok.size() >= 8) endedAddRow({tok[2], tok[3], tok[4], tok[5], tok[6], tok[7]});
        return;
    }
    if (ev == "ENDED_END") {
    ...
    if (ev == "CHAT_USERS") {
        ...
        for (int i = startIdx; i < tok.size(); ++i) {
            const QString u = tok[i].trimmed();
            if (!u.isEmpty() && u != username) users << u;
        }
        ...
        if (currentRoomId != 0) {
            if (okN) setRoomParticipants(n);
            else if (!username.isEmpty()) setRoomParticipants(users.size() + 1);
        }

        if (ui->lstRoomUsers) ui->lstRoomUsers->addItems(users);
        if (ui->lstChatUsers) ui->lstChatUsers->addItems(users);
    ...
    if (ev == "CHAT_FROM") {
        ...
        QString payload = raw;
        if (payload.startsWith(prefix)) payload = payload.mid(prefix.size());
    ...
    if (ev == "ROOM_CHAT") {
        ...
        if (from.isEmpty() || msg.isEmpty()) return;
...
void MainWindow::openChatWith(const QString& peer, bool switchTab) {
    ...
    if (p.isEmpty() || p == username) return;

    m_chatPeer = p;
    if (ui->lblChatPeer) ui->lblChatPeer->setText(QString("Đang chat với: %1").arg(m_chatPeer));
    if (ui->btnChatSend) ui->btnChatSend->setEnabled(true);
    ...
    if (ui->edtChatMsg) ui->edtChatMsg->setFocus();
}

void MainWindow::renderChat(const QString& peer) {
    if (!ui->txtChat) return;
    ui->txtChat->clear();
    const auto history = m_chatHistory.value(peer);
    for (const auto& line : history) ui->txtChat->appendPlainText(line);
}
...
void MainWindow::appendChatLine(const QString& peer, const QString& line) {
    ...
    if (m_chatPeer.compare(p, Qt::CaseInsensitive) == 0) {
        if (ui->txtChat) ui->txtChat->appendPlainText(line);
    } else {
...
void MainWindow::onChatSend() {
    if (!sock || !sock->isConnected()) return;
    if (m_chatPeer.isEmpty()) return;
    if (!ui->edtChatMsg) return;
...
void MainWindow::renderSelectedRoomUsers() {
    ...
    for (const QString& u : std::as_const(m_selectedRoomUsers)) {
        ...
        if (!q.isEmpty() && !u.contains(q, Qt::CaseInsensitive)) continue;
